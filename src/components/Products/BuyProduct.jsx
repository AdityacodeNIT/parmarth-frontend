...
const BuyProduct = () => {
  const { totalProductPrice, productDescription, setAddressId } = useContext(UserContext);
      ...
      <div className="flex flex-col lg:flex-row justify-between p-4 bg-pink-50 rounded-lg shadow-md mt-4">
        ...
        <div className="w-full lg:w-1/2 p-4">
          <div className="border-4 p-4 border-gray-600 rounded-lg shadow-lg bg-white">
            ...
            <div className="mt-4">{productDescription()}</div>
            <div className="flex justify-between mt-2 p-2 font-semibold text-xl border border-gray-300 rounded-lg">
              <div>Subtotal</div>
            ...
            <div className="flex justify-between mt-2 p-2 font-semibold text-xl border border-gray-300 rounded-lg">
              <div>Total</div>
              <div>₹{Math.ceil(totalProductPrice() * 1.18)}</div>
            </div>
            <Link to="/payments">
              ...
              <div className="bg-white p-4 rounded-lg shadow-md">
                ...
                <div className="bg-gray-900 text-white p-4 rounded-lg shadow-md">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm md:text-md">
                    ...
                    <div>
                      <strong>Alternate Phone:</strong> {addressDetails.alternateNumber || "N/A"}
                    </div>
                  </div>
                </div>
              </div>
            ) : (
              <p className="text-center text-gray-600">No address found. Please add one.</p>
            )}
